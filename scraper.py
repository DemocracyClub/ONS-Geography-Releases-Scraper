...
def init():
    ...
    scraperwiki.sql.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS onspd_id_unique ON onspd (id);"
    )
    scraperwiki.sql.execute(
        """
    ...
    scraperwiki.sql.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS onsad_id_unique ON onsad (id);"
    )
    scraperwiki.sql.execute(
        """
    ...
    scraperwiki.sql.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS lgd_id_unique ON lgd (id);"
    )
    scraperwiki.sql.execute(
        """
    ...
    scraperwiki.sql.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS wards_id_unique ON wards (id);"
    )
    scraperwiki.sql.execute(
        """
        CREATE TABLE IF NOT EXISTS lad_maps (
            url TEXT,
            title TEXT,
            id TEXT);"""
    )
    scraperwiki.sql.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS lad_maps_id_unique ON lad_maps (id);"
    )
...
def scrape(url, table):
    ...
    for result in results:
        ...
        scraperwiki.sqlite.save(unique_keys=["id"], data=record, table_name=table)
...
lad_maps_url = "https://opendata.arcgis.com/api/v2/datasets?filter%5Bcatalogs%5D=geoportal1-ons.opendata.arcgis.com&include=organizations%2Cgroups&page%5Bnumber%5D=1&page%5Bsize%5D=10&q=LAD%20CTY%20UA%20Map&sort=-updatedAt"
